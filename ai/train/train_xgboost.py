...
# ── 경로 설정 ──────────────────────────────────
# 어느 폴더에서 실행해도 backend/ 기준으로 동작
BASE_DIR    = os.path.dirname(os.path.abspath(__file__))
BACKEND_DIR = os.path.join(BASE_DIR, "..", "backend")
CSV_PATH    = os.path.join(BACKEND_DIR, "data", "sensor_log.csv")
RESULT_DIR  = os.path.join(BACKEND_DIR, "models")
...
feature_cols = [
    ...
    "recent_mean_power_3", "recent_mean_power_6",
    "power_trend_3", "power_diff_1", "power_std_6"
]

optional_cols = ["battery_voltage", "battery_current", "battery_power", "soil", "soc"]
...
plt.plot(result_df["timestamp"], result_df["actual_power_1h"], label="Actual Power",    linewidth=2)
plt.plot(result_df["timestamp"], result_df["pred_power_1h"],   label="Predicted Power", linewidth=2)
plt.title("XGBoost: Actual vs Predicted Solar Power (1 Hour Ahead)")
plt.xlabel("Timestamp"); plt.ylabel("Solar Power (W)")
plt.legend(); plt.xticks(rotation=45); plt.tight_layout()
plt.savefig(os.path.join(RESULT_DIR, "xgboost_actual_vs_pred.png"), dpi=300)
plt.show()
print(f"그래프 저장 → {RESULT_DIR}/xgboost_actual_vs_pred.png")
...
plt.figure(figsize=(14, 6))
plt.plot(plot_df["timestamp"], plot_df["actual_power_1h"], label="Actual Power",    linewidth=3)
plt.plot(plot_df["timestamp"], plot_df["pred_power_1h"],   label="Predicted Power", linewidth=3)
plt.title("XGBoost Prediction (Last 50 Samples)")
plt.xlabel("Timestamp"); plt.ylabel("Solar Power (W)")
plt.legend(); plt.xticks(rotation=45); plt.tight_layout()
plt.savefig(os.path.join(RESULT_DIR, "xgboost_actual_vs_pred_last50.png"), dpi=300)
plt.show()
print(f"발표용 그래프 저장 → {RESULT_DIR}/xgboost_actual_vs_pred_last50.png")
...
plt.savefig(os.path.join(RESULT_DIR, "xgboost_feature_importance.png"), dpi=300)
plt.show()
print(f"Feature 중요도 저장 → {RESULT_DIR}/xgboost_feature_importance.png")
